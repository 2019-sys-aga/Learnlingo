...
import { Flashcard, Question, ChatMessage } from '../types'
...
class AIService {
  ...
  private initializeModel() {
    ...
    const apiKey = localStorage.getItem('jungle_ai_api_key') || import.meta.env.VITE_GEMINI_API_KEY
    if (!apiKey) {
  ...
  async explainConcept(concept: string, context?: string): Promise<string> {
    try {
      this.initializeModel()
      let prompt = `Please explain the concept of "${concept}" in a clear, educational way.`
      
      if (context) {
        prompt += ` Use the following context to provide a relevant explanation: ${context}`
      }
  ...
  async generatePracticeQuestion(topic: string, difficulty: 'easy' | 'medium' | 'hard' = 'medium'): Promise<Question> {
    try {
      this.initializeModel()
      const prompt = `
        Generate a ${difficulty} practice question about ${topic}.
      ...
      throw new Error('Failed to generate practice question. Please try again.')
    }
  }
}
